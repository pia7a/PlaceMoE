...
@register_router_extractor("Qwen3VLMoeTextTopKRouter")
...
class MoERouterMonitor:
    ...
    def __init__(
        ...
        # Last reduced interval, kept for structured logging by callers.
        self._last_counts_matrix: Optional[torch.Tensor] = None
        self._last_load_matrix: Optional[torch.Tensor] = None
        self._last_rank_to_rank_matrix: Optional[torch.Tensor] = None
    ...
    def _stack_and_reduce(self) -> torch.Tensor:
        ...
        self._last_rank_to_rank_matrix = self._build_rank_to_rank_matrix(matrix)
    ...
    def _build_rank_to_rank_matrix(self, local_counts: torch.Tensor) -> Optional[torch.Tensor]:
        """Gather local per-destination-rank token-expert counts across EP ranks.

        Returns a CPU tensor with shape ``[num_moe_layers, ep_size, ep_size]``:
        layer, source EP rank, destination EP rank. Counts are top-k expanded
        token-expert assignments, matching the dispatch load used by EP all-to-all.
        """
        if self.ep_group is None or not dist.is_initialized():
            return None
        ep_size = dist.get_world_size(self.ep_group)
        if ep_size <= 1 or self.num_experts % ep_size != 0:
            return None
        num_local_experts = self.num_experts // ep_size
        local_dest = local_counts.reshape(local_counts.shape[0], ep_size, num_local_experts).sum(dim=2).contiguous()
        gathered = [torch.empty_like(local_dest) for _ in range(ep_size)]
        dist.all_gather(gathered, local_dest, group=self.ep_group)
        return torch.stack(gathered, dim=1).cpu()
    ...
    def get_load_matrix(self, current_step: int = 0) -> torch.Tensor:
        ...
        if matrix.numel() == 0:
            return matrix.float()
        self._last_counts_matrix = matrix.detach().cpu()
        matrix = matrix.float().cpu()
        row_sums = matrix.sum(dim=1, keepdim=True).clamp(min=1.0)
        matrix = matrix / row_sums
        self._last_load_matrix = matrix.detach().cpu()
        self._last_step_range = (self._accumulate_start_step, current_step)
        self._reset_counts()
        self._accumulate_start_step = current_step + 1
        return matrix

    def get_last_structured_payload(self) -> Dict[str, Any]:
        """Return JSON-serializable matrices from the most recent interval."""
        if self._last_counts_matrix is None or self._last_load_matrix is None:
            return {}
        load_matrix = self._last_load_matrix
        vio = self.compute_vio(load_matrix)
        payload: Dict[str, Any] = {
            "step_range": list(self._last_step_range),
            "num_layers": int(load_matrix.shape[0]),
            "num_experts": int(load_matrix.shape[1]),
            "expert_counts": self._last_counts_matrix.tolist(),
            "expert_load": load_matrix.tolist(),
            "max_vio": vio["max_vio"].tolist(),
            "min_vio": vio["min_vio"].tolist(),
            "avg_vio": vio["avg_vio"].tolist(),
        }
        if self._last_rank_to_rank_matrix is not None:
            payload["rank_to_rank_token_expert_counts"] = self._last_rank_to_rank_matrix.tolist()
        return payload
    ...
    def compute_metrics(
        ...
        **Collective**: under EP/DP this calls ``all_reduce``. Every rank in
        ...
        try:
            ...
            # Matplotlib is optional in lean runtime images; keep scalar imbalance
            # metrics rather than failing a training step for the image summary.
            pass
