...
def create_profiler(
    ...
    def should_export_trace() -> bool:
        raw = os.environ.get("VEOMNI_TORCH_PROFILE_EXPORT_RANKS", "0").strip()
        if raw.lower() == "all":
            return True
        ranks = {item.strip() for item in raw.split(",") if item.strip()}
        return str(global_rank) in ranks

    def handler_fn(p):
        if not should_export_trace():
            logger.info(f"Profiling trace export skipped on rank {global_rank}.")
            return
    ...
    if IS_NPU_AVAILABLE:
        ...
        requested_activities = os.environ.get("VEOMNI_TORCH_PROFILE_ACTIVITIES", "cpu,cuda").lower()
        activities = []
        if "cpu" in requested_activities:
            activities.append(profiler_module.ProfilerActivity.CPU)
        if "cuda" in requested_activities:
            activities.append(profiler_module.ProfilerActivity.CUDA)
        if not activities:
            activities = [profiler_module.ProfilerActivity.CUDA]
