...
class Qwen3VLMoeVisionPatchEmbed(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        profile_meta = {
            "module_name": "model.visual.patch_embed",
            "module_type": type(self).__name__,
            "layer_idx": None,
            "tokens": int(hidden_states.shape[0]) if hidden_states.ndim > 0 else int(hidden_states.numel()),
            "input_shape": list(hidden_states.shape),
            "input_dtype": str(hidden_states.dtype),
            "target_dtype": str(target_dtype),
            "input_device": str(hidden_states.device),
            "input_is_cuda": bool(hidden_states.is_cuda),
            "input_is_contiguous": bool(hidden_states.is_contiguous()),
        }

        def profile_range(section: str):
            if profiler is None:
                return contextlib.nullcontext()
            return profiler.cuda_range(section, record_type="module_forward", metadata=profile_meta)

        flat_patch_dim = self.in_channels * self.temporal_patch_size * self.patch_size * self.patch_size
        with profile_range("vision_patch_embed_flat_view"):
            hidden_states = hidden_states.view(-1, flat_patch_dim)
        with profile_range("vision_patch_embed_to_dtype"):
            hidden_states = hidden_states.to(dtype=target_dtype)
        with profile_range("vision_patch_embed_linear"):
            hidden_states = F.linear(hidden_states, self.proj.weight.view(self.embed_dim, -1), self.proj.bias)
        return hidden_states
