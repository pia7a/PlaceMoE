...
try:
    ...
    def is_flash_attention_requested(config):
        return "flash" in str(getattr(config, "_attn_implementation", "")).lower()

    def maybe_autocast(device_type=None, enabled=True, **kwargs):
        if not enabled:
            return nullcontext()
        return torch.autocast(device_type=device_type, enabled=enabled, **kwargs)
...
def _qwen3_vl_async_ulysses_attention_forward(
    ...
    **kwargs,
    ...
    attention_interface = _get_attention_interface(self.config._attn_implementation)
...
def _get_attention_interface(attn_implementation: str) -> Callable:
    ...
    return ALL_ATTENTION_FUNCTIONS.get(attn_implementation) or eager_attention_forward
...
class Qwen3VLMoeTextAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        attention_interface: Callable = _get_attention_interface(self.config._attn_implementation)
...
class Qwen3VLMoeTextRotaryEmbedding(nn.Module):
    ...
    def compute_default_rope_parameters(
        ...
        base = getattr(config, "rope_theta", rope_parameters.get("rope_theta", 10000.0))
...
class Qwen3VLMoeForConditionalGeneration(Qwen3VLMoePreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        output_fields = getattr(Qwen3VLMoeCausalLMOutputWithLogProbs, "__dataclass_fields__", {})
        if output_fields:
            output_kwargs = {key: value for key, value in output_kwargs.items() if key in output_fields}
        return Qwen3VLMoeCausalLMOutputWithLogProbs(**output_kwargs)
