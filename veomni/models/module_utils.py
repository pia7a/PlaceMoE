    ...
    **kwargs,
    ...
    def _chunk_and_broadcast_and_dispatch(name, shape, dtype, tensor):
        ...
        if global_rank == 0:
            ...
            allow_redundant_slot_padding = _is_hiermoe_redundant_slot_expert_param(
                para_group_name,
                name,
                tensor.shape,
                target_shape,
            ...
            base_chunk_dim0 = tensor.shape[0] // para_size
            if allow_redundant_slot_padding:
                assert base_chunk_dim0 <= target_shape[0], (
                    f"Parallel {para_group_name}: padded shape {shape} at dim 0 // {para_size} == {base_chunk_dim0}, "
                    f"larger than target local dim {target_shape[0]}"
                )
            else:
                assert base_chunk_dim0 == target_shape[0], (
                    f"Parallel {para_group_name}: padded shape {shape} at dim 0 // {para_size} == {base_chunk_dim0}, "
                    f"not equal to target local dim {target_shape[0]} for {name}."
                )
            assert tuple(tensor.shape[1:]) == tuple(target_shape[1:]), (
                f"Parallel {para_group_name}: tensor tail shape {tuple(tensor.shape[1:])} does not match "
                f"target tail shape {tuple(target_shape[1:])} for {name}."
            )
        ...
        for chunk_id in range(para_size):
            ...
            if dist.get_rank() == 0:
                chunk_tensor = chunk_loaded_data[chunk_id].contiguous()
                if tuple(chunk_tensor.shape) == tuple(broadcast_buffer.shape):
                    broadcast_buffer.copy_(chunk_tensor)
                else:
                    assert allow_redundant_slot_padding, (
                        f"Parallel {para_group_name}: chunk shape {tuple(chunk_tensor.shape)} does not match "
                        f"target broadcast buffer shape {tuple(broadcast_buffer.shape)} for {name}."
                    )
                    broadcast_buffer.zero_()
                    broadcast_buffer[: chunk_tensor.shape[0]].copy_(chunk_tensor)
