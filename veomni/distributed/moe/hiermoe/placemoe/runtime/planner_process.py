...
def launch_planner_process(
    ...
    *,
    ...
    # Execute the supervisor by file path.  ``python -m veomni...`` imports
    # the complete VeOmni package before starting the planner, which can add
    # another accelerator-runtime initialization to every hot update.
    supervisor = Path(__file__).with_name("planner_supervisor.py")
    supervised_command = [sys.executable, str(supervisor), "--", *command]
