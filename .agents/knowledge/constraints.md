...
8a. **Replicated experts count once in the global gradient norm**
   - HierMoE redundant-gradient synchronization writes the summed logical gradient into every physical copy so all copies receive the same optimizer update.
   - FSDP2 gradient-norm reduction must include exactly one owner slot per logical expert; counting every synchronized copy changes the clip coefficient.
   - Apply the resulting global clip coefficient to every physical copy. Do not zero replica gradients before the optimizer step.
   - Swap/replica placement does not support FSDP2 CPU offload; reject the configuration before model construction because HCCL/NCCL placement collectives and payload migration require accelerator-resident expert state.

8b. **Custom trainer paths must preserve the HierMoE lifecycle**
   - Every trainable model forward must run inside `BaseTrainer.hiermoe_layer_swap_forward()` so current-route layer planning is enabled only for the intended forward, not checkpoint recomputation or reference-model inference.
   - Frozen/reference models that share the global EP process must be built and executed inside `BaseTrainer.hiermoe_placement_disabled()`. They retain token deduplication but must not reserve redundant slots or use the trainable model's placement mapping.
   - Every optimizer step must reach the idempotent `BaseTrainer.run_hiermoe_expert_swap()` step-end fallback. Otherwise step-mode route samples and placement timing queues are never drained.
...
17. **Checkpoint save/load requires all ranks to participate**
    ...
    - HierMoE redundant-slot budgets must match between save and resume. Rank-local extra state is preflighted before DCP loads model or optimizer tensors because compact and slot-expanded expert tensors have different shapes.
