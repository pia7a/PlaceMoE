...
- `hiermoe_layer_swap_forward()` -> scopes current-route placement to the trainable forward
- `hiermoe_placement_disabled()` -> keeps token dedup active while isolating frozen/reference models from trainable placement state
- `run_hiermoe_expert_swap()` -> idempotent step-boundary planning, also invoked by the common step-end callback path

Subclasses override specific methods (e.g., `compute_loss()`, custom data transforms) rather than the entire training loop. Custom trainers that issue their own model forward must use the shared HierMoE forward context; delegating `on_step_end()` to `BaseTrainer` guarantees step-mode queues are drained even when the custom `train_step()` omits an explicit placement call.
