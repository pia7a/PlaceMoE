#!/usr/bin/env bash
# Qwen3-VL MoE standalone VeOmni pretraining with MoE monitor/timing JSONL.
set -euo pipefail
...
RUN_NAME=${RUN_NAME:-qwen3vl_a3b_veomni_pretrain_ep8_60step_$(date +%Y%m%d_%H%M%S)}
RUN_ROOT=${RUN_ROOT:-"${REPO_ROOT}/pretrain_runs/${RUN_NAME}"}
MODEL_PATH=${MODEL_PATH:-/model/Qwen3-VL-30B-A3B-Instruct-layer24}
...
EP_SIZE=${EP_SIZE:-8}
NUM_MOE_LAYERS=${NUM_MOE_LAYERS:-16}
...
VEOMNI_FULL_PROFILE_ENABLE=${VEOMNI_FULL_PROFILE_ENABLE:-0}
VEOMNI_FULL_PROFILE_DIR=${VEOMNI_FULL_PROFILE_DIR:-"${VERL_MOE_PROFILE_DIR}/full_timing"}
VEOMNI_FULL_PROFILE_START_STEP=${VEOMNI_FULL_PROFILE_START_STEP:-5}
...
VEOMNI_TORCH_PROFILE_ACTIVITIES=${VEOMNI_TORCH_PROFILE_ACTIVITIES:-cpu,cuda}

mkdir -p "${RUN_ROOT}" "${VERL_MOE_MONITOR_DIR}" "${VERL_MOE_TIMING_DIR}"
if [[ "${VEOMNI_FULL_PROFILE_ENABLE}" == "1" || "${VEOMNI_FULL_PROFILE_ENABLE}" == "true" ]]; then
    mkdir -p "${VEOMNI_FULL_PROFILE_DIR}"
fi
if [[ "${VEOMNI_TORCH_PROFILE_ENABLE}" == "1" || "${VEOMNI_TORCH_PROFILE_ENABLE}" == "true" ]]; then
    mkdir -p "${VEOMNI_TORCH_PROFILE_DIR}"
fi

cat > "${RUN_ROOT}/launch_env.txt" <<EOF
RUN_NAME=${RUN_NAME}
RUN_ROOT=${RUN_ROOT}
...
VEOMNI_PROFILE_ROOT=${VEOMNI_PROFILE_ROOT}
VERL_MOE_PROFILE_DIR=${VERL_MOE_PROFILE_DIR}
VERL_MOE_MONITOR_DIR=${VERL_MOE_MONITOR_DIR}
VERL_MOE_TIMING_DIR=${VERL_MOE_TIMING_DIR}
VERL_MOE_TIMING_NUM_LAYERS=${VERL_MOE_TIMING_NUM_LAYERS}
VEOMNI_FULL_PROFILE_ENABLE=${VEOMNI_FULL_PROFILE_ENABLE}
VEOMNI_FULL_PROFILE_DIR=${VEOMNI_FULL_PROFILE_DIR}
...
VEOMNI_FULL_PROFILE_RUN_KIND=${VEOMNI_FULL_PROFILE_RUN_KIND}
VEOMNI_TORCH_PROFILE_ENABLE=${VEOMNI_TORCH_PROFILE_ENABLE}
VEOMNI_TORCH_PROFILE_DIR=${VEOMNI_TORCH_PROFILE_DIR}
VEOMNI_TORCH_PROFILE_START_STEP=${VEOMNI_TORCH_PROFILE_START_STEP}
VEOMNI_TORCH_PROFILE_END_STEP=${VEOMNI_TORCH_PROFILE_END_STEP}
VEOMNI_TORCH_PROFILE_RANK0_ONLY=${VEOMNI_TORCH_PROFILE_RANK0_ONLY}
VEOMNI_TORCH_PROFILE_EXPORT_RANKS=${VEOMNI_TORCH_PROFILE_EXPORT_RANKS}
VEOMNI_TORCH_PROFILE_RECORD_SHAPES=${VEOMNI_TORCH_PROFILE_RECORD_SHAPES}
VEOMNI_TORCH_PROFILE_PROFILE_MEMORY=${VEOMNI_TORCH_PROFILE_PROFILE_MEMORY}
VEOMNI_TORCH_PROFILE_WITH_STACK=${VEOMNI_TORCH_PROFILE_WITH_STACK}
VEOMNI_TORCH_PROFILE_WITH_MODULES=${VEOMNI_TORCH_PROFILE_WITH_MODULES}
VEOMNI_TORCH_PROFILE_ACTIVITIES=${VEOMNI_TORCH_PROFILE_ACTIVITIES}
EOF
