#!/usr/bin/env bash
# Qwen3-VL-30B-A3B full 48-layer four-node Ascend NPU 400-step light profile.
set -euo pipefail
...
RUN_NAME=${RUN_NAME:-qwen3vl_full48_sharegpt4v_ep32_lightprof_400step_$(date +%Y%m%d_%H%M%S)}
...
# Warm up steps 1-2, then sample every 10 steps. Full module timing is limited
# to global rank 0 to keep the 400-step run stable and lightweight; MoE timing
# and monitor files still cover all ranks for communication/expert analysis.
...
if [[ ! -e "${DATA_PATH}" ]]; then
    ...
    echo "Run profile/scripts/prepare_sharegpt4v_coco_abs.py first." >&2
    exit 2
fi
