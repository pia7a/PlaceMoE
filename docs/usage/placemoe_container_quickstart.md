...
Clone the VeOmni fork that contains PlaceMoE on every training node, and check
out the same validated PlaceMoE tag or commit. Do not run one distributed job
from different revisions.

```bash
git clone https://github.com/pia7a/VeOmni.git /home/user/VeOmni
cd /home/user/VeOmni
...
| --- | --- | --- | --- |
| `SOURCE_ROOT` | `/workspace/VeOmni` | Read-only | VeOmni source with PlaceMoE at one pinned commit |
...
```bash
git -C /home/user/VeOmni rev-parse HEAD
...
environment installed in the image. Mounting it at `/workspace/VeOmni` and
...
The final command must resolve under `/workspace/VeOmni`; otherwise the
...
this repository targets Transformers 5.9.0, FSDP2, and VeOmni's stacked EP
...
1. Add the model to VeOmni using the Transformers v5 model workflow. Register
   ...
   from the pinned Transformers 5.9.0 implementation. Do not edit files under
...
`/workspace/VeOmni`, set
`PYTHONPATH=/workspace/VeOmni:/opt/placemoe`, and verify the imported module
