...
在每个训练节点上克隆包含 PlaceMoE 的 VeOmni fork，并检出相同的已验证
PlaceMoE 标签或提交。一次分布式任务不能混用不同的源码版本。
...
| 宿主机变量 | 容器路径 | 权限 | 内容 |
| --- | --- | --- | --- |
| `SOURCE_ROOT` | `/workspace/VeOmni` | 只读 | 固定到同一个 commit 的 VeOmni 与 PlaceMoE 源码 |
...
将源码挂载到 `/workspace/VeOmni`，并把该路径放在 `PYTHONPATH` 首位，可以
...
最后一条命令的输出必须位于 `/workspace/VeOmni` 下；否则容器使用的是镜像
构建时写入的源码，而不是当前挂载的源码。
...
5.9.0、FSDP2 和 VeOmni 的堆叠 EP kernel。因此，不能在未进行 VeOmni 模型移植
的情况下，直接用 PlaceMoE 训练该 remote-code checkpoint。

建议按以下顺序进行生产级适配：

1. 按照 VeOmni Transformers v5 模型流程添加该模型。在 import 阶段注册
   `pangu_ultra_moe`，并让生成的模型代码基于固定的 Transformers 5.9.0 实现。
...
不要覆盖挂载 `/opt/placemoe`。将源码挂载到 `/workspace/VeOmni`，设置
`PYTHONPATH=/workspace/VeOmni:/opt/placemoe`，并按第 2.1 节检查实际导入路径。
