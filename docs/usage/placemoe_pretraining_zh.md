...
干净宿主机中自动安装 CANN、PyTorch 或 torch-npu。`docker/ascend/` 下的
Dockerfile 遵循相同的插件边界：基础镜像提供加速器环境和通用 VeOmni 依赖，
uv 则构建并安装当前 PlaceMoE 源码。
...
`failure_policy: continue` 会记录 planner 失败并保留当前决策。当自适应更新失败
必须终止任务时，使用 `raise`。
...
- 论文工作流保存在 `scripts/placemoe/reproduction/` 和 `docs/perf/` 下，不属于
  生产 API。
